...
class A2C(OnPolicyAlgorithm):
    ...
    def train(self) -> None:
        ...
        for rollout_data in self.rollout_buffer.get(batch_size=None):
            ...
            # 注意：这里所求values不是Q_values，看似Q(St, At)，但不是！
            values, log_prob, entropy = self.policy.evaluate_actions(rollout_data.observations, actions)
            print(f"old_values = {rollout_data.old_values}")
            print(f"new_values = {values.squeeze()}")
            print(f"validation if old_values == new_values {rollout_data.old_values == values.flatten()}")
            # print(f"shape of old_values is {rollout_data.old_values.shape}")
            # print(f"shape of new_values is {values.squeeze().shape}")
