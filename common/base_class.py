...
class BaseAlgorithm(ABC):
    ...
    :param use_sde: Whether to use generalized State Dependent Exploration (gSDE)
        ...
        # 是否使用广义状态依赖探索（gSDE）替代动作噪声探索（默认值：False）
    ...
    def __init__(
        ...
        # policy_class类定义，而不是实例化，其实例化在上一层中进行
        ...
        self._episode_num = 0
        # Used for gSDE only
        self.use_sde = use_sde # 决定是否开启sde
        self.sde_sample_freq = sde_sample_freq # 决定sde的采样频率
    ...
    def _init_callback(
        ...
        """
        :param callback: Callback(s) called at every step with state of the algorithm. # 在算法运行的每一步都会调用的回调函数
        :param progress_bar: Display a progress bar using tqdm and rich. # 使用 tqdm 和 rich 库显示进度条
        :return: A hybrid callback calling `callback` and performing evaluation. # 一个混合回调，既调用 `callback` 也执行评估操作
        """
    ...
    def _setup_learn(
        ...
        :param callback: Callback(s) called at every step with state of the algorithm.          
            #### 在算法运行的每一步都会调用的回调函数。env.step(action) - 与环境互动的每一步，都会调用？
        :param reset_num_timesteps: Whether to reset or not the ``num_timesteps`` attribute     
            #### 是否重置 ``num_timesteps`` 属性
        :param tb_log_name: the name of the run for tensorboard log                             
            #### 用于TensorBoard日志的运行名称
        ...
        if self.ep_info_buffer is None or reset_num_timesteps:
            ...
            # collections.deque - 1.双端队列，可以在头尾进行数据操作；2.固定长度为maxlen的滑动窗口
            # https://www.kdocs.cn/l/caeJ9Tgq2Azj?linkname=W4QwFbNgZN
        ...
        if reset_num_timesteps or self._last_obs is None:
            ...
            self._last_obs = self.env.reset()  # type: ignore[assignment] # 在rollout_buffer计算中，没看到self._last_obs初始化，在这里看到了
