...
class BaseBuffer(ABC):
    ...
    def __init__(
        ...
        super().__init__()
        self.buffer_size = buffer_size # 设置buffer_size大小
        self.observation_space = observation_space # 设置状态空间类型
        self.action_space = action_space # 设置动作空间类型
        self.obs_shape = get_obs_shape(observation_space)  # type: ignore[assignment] # 获取状态空间维度

        self.action_dim = get_action_dim(action_space) # 获取动作空间维度
        self.pos = 0
        self.full = False
...
class RolloutBuffer(BaseBuffer):
    ...
    # 用途定位：这是一个专为 on-policy 算法（如 A2C、PPO）设计的经验缓冲区。

    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
        # On-policy vs Off-policy：
            # On-policy：训练数据必须由当前策略生成（不能复用旧策略的数据）。
            # 因此，buffer 只临时存储最新一轮交互数据，用完即弃。
            # 对比 off-policy（如 DQN、SAC）：经验可长期存储、反复采样。
        # On-policy 的典型流程：
            # collect SARSA with old policy → train and update policy → discard SARSA → collect SARSA again with new policy → ...

        # transitions: SAR-SAR-SAR-SAR....这就是有名的SARSA，一个R隔开两个时刻的SA，t时刻SA在R前，t+1时刻SA在R之后
    
    This experience will be discarded after the policy update.
    # 生命周期：这批经验在完成一次策略更新后就会被丢弃，不会重复使用。

    In order to use PPO objective, we also store the current value of each state
    and the log probability of each taken action.
        # 额外存储内容：为了实现 PPO 的目标函数，除了基本的 (s, a, r)，还必须保存：
            # 状态价值 V(s_t)（由当前 critic 网络估计）
            # 动作对数概率 log π(a_t | s_t)（由当前 actor 网络输出）
                # 为什么需要上面两个？
                    # PPO 使用 重要性采样比（importance sampling ratio）：
                    # r_t(θ) = π_θ(a_t|s_t) / π_old(a_t|s_t) = exp(log π_θ - log π_old)
                    # 所以必须在收集数据时记录 log π_old(a_t|s_t)（即“旧策略”的 log prob）
                    # 同样，GAE 计算优势函数 A_t 需要 V(s_t)

    The term rollout here refers to the model-free notion and should not
    be used with the concept of rollout used in model-based RL or planning.
    # 术语澄清：这里的 “rollout” 是 model-free RL 中的轨迹收集，不是 model-based RL 或规划（planning）中的 “rollout”。

    Hence, it is only involved in policy and value function training but not action selection.
    # 功能边界：该 buffer 仅用于训练策略和价值函数，不参与动作选择（action selection）过程。
    ...
    def compute_returns_and_advantage(self, last_values: th.Tensor, dones: np.ndarray) -> None:
        ...
        for step in reversed(range(self.buffer_size)):
            ...
            if step == self.buffer_size - 1:
                ...
                next_values = last_values # 最后一个状态的价值估计
            else:
                next_non_terminal = 1.0 - self.episode_starts[step + 1]
                next_values = self.values[step + 1]

            # TD原理:
                # V_t+1(S_t) = V_t(St) - α_t * [V_t(St) - (R + γ * V_t(S_t+1))]
                # γ折扣因子 - self.gamma
                # α学习率   - self.gamma * self.gae_lambda
                # TD_target - R + γ * V_t(S_t+1)
                # TD_error - V_t(St) - (R + γ * V_t(S_t+1))

            # TD eror计算
            delta = self.rewards[step] + self.gamma * next_values * next_non_terminal - self.values[step]
            # 递推公式，计算GAE代替优势函数advantage —— 贝尔曼方程原理
