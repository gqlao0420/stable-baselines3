...
class DiagGaussianDistribution(Distribution):
    ...
    def proba_distribution_net(self, latent_dim: int, log_std_init: float = 0.0) -> tuple[nn.Module, nn.Parameter]:
        ...
        # https://www.kdocs.cn/l/caeJ9Tgq2Azj?linkname=V4wO1CCTb5
    ...
    def proba_distribution(
        ...
        #### 根据其参数（均值、标准差）创建该分布。
        #### 因为每一个env.step时，obs都会不同，所以对应的策略分布均值会不同，但在没有更新策略网络参数前，网络内部参数都是一样的
        #### 但是log_std标准差是网络参数，相同episode保持不变，只有optimizer.step后会更新

        :param mean_actions:
        :param log_std:
        :return:
        """
        # 构造一个与 mean_actions 形状相同的标准差张量（standard deviation）
        action_std = th.ones_like(mean_actions) * log_std.exp()
        # 构建均值为 mean_actions，标准差为action_std的高斯分布
...
def make_proba_distribution(
    ...
    if isinstance(action_space, spaces.Box):
        cls = StateDependentNoiseDistribution if use_sde else DiagGaussianDistribution
            ...
            # 如果use_sde == True，则使用StateDependentNoiseDistribution，反之则使用DiagGaussianDistribution
