...
class OnPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_model(self) -> None:
        # 定义：1.学习率、布种子、2.rollout_buffer类定义及实例化、3.policy实例化
        
        # 1. OnPolicyAlgorithm，继承父类BaseAlgorithm的方法
        self._setup_lr_schedule()
        self.set_random_seed(self.seed)

        # 2. 根据状态空间类型是离散的还是连续的区别，设置rollout_buffer_class的类型
        if self.rollout_buffer_class is None:
            if isinstance(self.observation_space, spaces.Dict):
                self.rollout_buffer_class = DictRolloutBuffer # 对应离散状态空间
            else:
                self.rollout_buffer_class = RolloutBuffer # 对应连续状态空间

        # 初始化rollout_buffer_class实例
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                # th.no_grad() - 不追踪梯度，那actions, values, log_probs就不能做梯度反向传播
