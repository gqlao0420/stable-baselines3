...
class BaseModel(nn.Module):
    ...
    def set_training_mode(self, mode: bool) -> None:
        ...
        :param mode: if true, set to training mode, else set to evaluation mode
           # 设置模型状态是训练模式还是评估模式，mode: true - training, False - evaluation
        """
        self.train(mode)
...
class ActorCriticPolicy(BasePolicy):
    ...
    def __init__(
        ...
        # 返回一个特征提取的class
        # 策略网络和价值网络由“主干网络(原始特征提取器+隐藏层特征提取网络) + 输出头”构成，其中主干网络结构可以共享，也可以不共享
       
        # 控制 Actor 和 Critic 是否共享主干网络（backbone）的核心逻辑
        # self.features_extractor早在 BaseModel 初始化中有声明了

        # 定义主干网络 ———— (策略和价值隐)原始特征提取器
        ...
        if self.share_features_extractor:
            # 策略和价值特征提取器共享一个class的结构和参数
            self.pi_features_extractor = self.features_extractor
            self.vf_features_extractor = self.features_extractor
        else:
            # 策略和价值特征提取器分别采用不同class的结构和参数
            self.pi_features_extractor = self.features_extractor
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        distribution = self._get_action_dist_from_latent(latent_pi) # ← 基于当前 obs 计算 π(a|s)
        # 动作采样
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor) -> Distribution:
        ...
        Retrieve action distribution given the latent codes.
            #### 从函数名字很明显就能看出来，它的作用是从当前状态obs，估计π(a|s)，即策略的分布
            #### 比如DiagGaussianDistribution中，从obs中提取
        ...
        :return: Action distribution
            ...
            #### 在这里有一个误区：self.log_std是怎么更新的？请看链接：https://www.kdocs.cn/l/caeJ9Tgq2Azj?linkname=V4wO1CCTb5
            #### 简单来说，它是神经网络的一个可学习参数，从神经网络只能够传递过来，在每一个在不进行 optimizer.step() 之前，都是相同的。
            #### 而 mean_actions 是随着每一个环境步在更新，也是可学习的，更新频率非常频繁。
